...
pub fn make_filter(filename: &str, frequency_threshold: Option<i32>) -> Result<HP, errors::DialettoError> {
    ...
    let tokens = BufReader::new(f)
        ...
        .filter_map(|line| {
            ...
            let word = sp.next().unwrap();
            let freq: i32 = sp.next().unwrap().parse().unwrap();
            if let Some(ft) = frequency_threshold {
                if freq >= ft {
                    Some(word.to_string())
                } else {
                    None
...
pub fn encode_to_file<T: AsRef<Path>>(filter: &HP, filename: T) -> Result<(), DialettoError> {
